...
SPLIT_NO_HYPHEN = re.compile(r"[\s,]+")
# NOTE: we removed the "treat fully-uppercase tokens as degrees" heuristic.
# UPPERCASE_DEG_RE is no longer used for degree detection.
UPPERCASE_DEG_RE = None
DASH_ONLY = {'-', '–', '—'}
...
def extract_degrees_from_part(part: str) -> list:
    """
    Return a list of normalized degrees found in a comma/space separated part.
    Only tokens that map via DEGREE_MAP are considered degrees now.
    """
    if not isinstance(part, str) or not part.strip():
        return []
    tokens = SPLIT_NO_HYPHEN.split(part.strip())
    ...
    for t in tokens:
        ...
        if key in DEGREE_MAP:
            out.append(DEGREE_MAP[key])
        # previously we also treated ALL-CAPS tokens as degrees — that is disabled now.
    return out
...
def parse_with_degree(raw: str):
    ...
    if not s:
        return '', '', '', 'NA'
    parts = [p.strip() for p in re.split(r"\s*,\s*", s) if p.strip()]
    # 1) Collect degrees from trailing comma-separated parts (only via DEGREE_MAP)
    degrees = []
    for p in parts[1:]:
        degrees.extend(extract_degrees_from_part(p))
    # 2) Remove pure-degree trailing parts so they don't affect name parsing
    # Now only remove parts if ALL tokens in the part are recognized in DEGREE_KEYS.
    while parts:
        cand = parts[-1]
        toks = [t for t in SPLIT_NO_HYPHEN.split(cand) if t and t not in DASH_ONLY]
        if toks and all((normalize_token(t) in DEGREE_KEYS) for t in toks):
            parts.pop()
        else:
            break
    # 3) Suffix handling (Jr, Sr, II, III, IV...)
    ...
    if len(parts) >= 2 and ',' in s:
        last_base = parts[0]
        # Use all trailing parts as the "right" side (handles "Last, MD, First, M.")
        right = strip_leading_titles(' '.join(parts[1:]))
        # split more robustly
        right_tokens = [t for t in re.split(r"[\s,]+", right) if t and t not in DASH_ONLY]
        kept_tokens = []
        for t in right_tokens:
            key = normalize_token(t)
            if key in DEGREE_MAP:
                ...
                # DO NOT treat ALL-CAPS token as degree anymore — leave it to name parsing
                kept_tokens.append(t)
        if len(kept_tokens) == 0:
            first, middle = '', ''
        elif len(kept_tokens) == 1:
        ...
        # "Firstname Middlename Lastname [Degrees]" (no comma)
        name2 = strip_leading_titles(' '.join(parts))
        tokens = [t for t in re.split(r"[\s,]+", name2) if t and t not in DASH_ONLY]
        kept = []
        for t in tokens:
            key = normalize_token(t)
            if key in DEGREE_MAP:
                ...
                # DO NOT treat ALL-CAPS token as degree anymore
                kept.append(t)
        if len(kept) == 0:
            medical_degree = ', '.join(dict.fromkeys(degrees)) if degrees else 'NA'
            return '', '', '', medical_degree
